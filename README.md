...
Все данные хранятся локально на диске. Никаких облаков и внешних сервисов, кроме самого Telegram.

## Два режима парсинга

Инструмент поддерживает два взаимозаменяемых способа сбора. Переключение — в GUI (**Настройки парсинга → Способ парсинга**) или флагом `--mode` в консоли. Обработка одинаковая в обоих режимах: те же триггеры, извлечение сущностей, категории, дедупликация и формат JSON.

**Telegram API (Telethon)** — классический userbot-режим через MTProto. Нужны `api_id`/`api_hash` с my.telegram.org и вход по коду. Полная функциональность: любые каналы, включая приватные, куда добавлен аккаунт.

**Веб (t.me/s)** — работает вообще без ключей и аккаунта. Telegram сам публикует веб-превью публичных каналов по адресу `https://t.me/s/<username>` — парсер читает эти страницы и листает историю пагинацией (`?before=<id>`). Ограничения:

- только публичные каналы (у которых есть @username);
- только текст постов, без медиа;
- приватные каналы и каналы с отключённым превью недоступны;
- между запросами выдерживается пауза 1.5–3 сек (вежливость к серверу).

Если получить api_id через my.telegram.org не удаётся — достаточно веб-режима.
![alt text](image.png)
...
минск, гомель, варшава, кишинёв, донецк, луганск, мариуполь,
симферополь, севастополь, николаев, полтава, сумы, чернигов,
донбасс, крым
```

**Приблизительная геолокация.** Если в посте нет координат, но упомянут город или регион из словаря `REGION_CENTERS` в `parser.py`, событие привязывается к центру этого города и помечается флагом `approximate: true`. В viewer такие события показываются с пометкой `≈`, пунктирным маркером и предупреждением в карточке. Явные координаты из текста всегда приоритетнее.
...
Нужна только для режима **Telegram API**. Веб-режим работает без всякой настройки.
...
- выбрать способ парсинга (**Настройки парсинга**: Telegram API или веб t.me/s без ключей),
- настроить API credentials (для API-режима),
...
# Веб-режим без API-ключей (публичные каналы)
python parser.py --mode web --from "вчера"

# Веб-режим по конкретным каналам
python parser.py --mode web --only "voenkor_xyz,osint_group" --from "3 days ago"
...
После каждого парсинга бот может присылать дайджест новых событий: категория,
локация, координаты, канал и ссылка на оригинальный пост. Работает в обоих
режимах парсинга.

Настройка (кнопка **Настройки парсинга**):

1. В Telegram у @BotFather создай бота командой `/newbot` — получишь токен вида `1234567890:AA...`.
2. Вставь токен в поле **Токен бота**.
3. Найди своего бота в Telegram и нажми **Start** (напиши ему `/start`).
4. Нажми **Определить** — chat_id подставится автоматически.

Настройки хранятся в `config.json` (`tg_bot_token`, `tg_chat_id`). Если событий нет — бот молчит.

## Автопарсинг по расписанию

Галка **⏱ Автоповтор каждые N ч** в нижней панели GUI запускает парсинг
автоматически с заданным интервалом (1–24 часа). Выбранный интервал
сохраняется в конфиге и восстанавливается при следующем открытии программы.
Пока окно открыто, инструмент работает как пост наблюдения: сам парсит,
сам докладывает в Telegram. Рядом отображается время следующего запуска.
...
├── web_fetch.py             # веб-режим: чтение превью t.me/s без ключей
├── notify.py                # дайджест новых событий в Telegram-бота
...
Возможности:
- таблица событий слева с сортировкой по дате, категории, локации, с колонкой координат (для приблизительных — с пометкой `≈`);
- интерактивная карта (Leaflet) справа с цветными маркерами по категориям; приблизительные локации — пунктирные и полупрозрачные;
- кластеризация маркеров: близкие события собираются в пузыри с числом, при приближении раскрываются;
- кнопка ⛶ — карта на весь экран;
- три базовые карты на выбор (переключатель в углу карты): **Тёмная** (CARTO, по умолчанию), **OSM**, **Спутник** (Esri). Тёмная выбрана по умолчанию потому, что сервер волонтёров OSM блокирует тайлы 403-м при открытии файла двойным кликом (`file://` — пустой Referer);
- фильтры по категории, стране, типу ракеты;
- полнотекстовый поиск по тексту поста, локации, каналу;
- панель деталей при клике на строку или маркер — полный текст, координаты, ссылка на оригинальный пост, ссылка на Google Maps. Панель всегда поверх карты, а центр карты смещается так, чтобы маркер не ушёл под панель.

Если слой OSM всё же понадобится — переключатель слоёв в правом верхнем углу карты.
